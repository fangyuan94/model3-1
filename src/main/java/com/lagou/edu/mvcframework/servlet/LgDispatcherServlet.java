...
public class LgDispatcherServlet extends HttpServlet {
    ...
    private Map<Handler,String[]> security = new HashMap<>(16);
    ...
    private void initSecurity() {
        ...
        handlerMapping.forEach(handler -> {
            ...
            if(lagouSecurity!= null){
                security.put(handler,lagouSecurity.value());
            }else{
                lagouSecurity = handler.getController().getClass().getAnnotation(LagouSecurity.class);
...
//        method.invoke() //
        ...
        securityIntercept(req,handler,resp);
    ...
    private void securityIntercept(HttpServletRequest req,Handler handler, HttpServletResponse resp) {

        String[] user = this.security.get(handler);
        if(user == null){
            return;
        }
        ...
        //当前用户没权限
        try {
            resp.getWriter().write("user no auth"+userName);
        } catch (IOException e) {
            e.printStackTrace();
        }
